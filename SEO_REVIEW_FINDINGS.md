...
The homepage exposes one JSON-LD graph containing WebSite, WebPage, Article, SoftwareSourceCode, Organization, Person, and DefinedTerm entities. The graph uses stable fragment IDs, links the WebPage to the WebSite, links the Article to the WebPage, connects ONYX to the BOUND Method DefinedTerm, identifies Soheil Mozaffari, and references the source repository at `https://github.com/SMozaff/Onyx-Framwork`.
