...
export default function SolutionsPage({locale}:{locale:Locale}) {
  ...
  return <div className={`solutions-page solutions-page--${theme}`} dir={rtl?"rtl":"ltr"}>
    ...
    <main>
      ...
      <section className="solutions-section"><div className="shell-content"><div className="solutions-heading"><div className="solutions-kicker"><span/>{t.scenarioTag}</div><h2>{t.scenarioTitleA}<br/><em>{t.scenarioTitleB}</em></h2><p>{t.scenarioBody}</p></div><div className="solutions-scenarios">{t.scenarios.map((scenario) => { const [n,sector,title,body,aLabel,aText,eLabel,eText,rLabel,rText] = scenario; return <article key={n}><header><span>{n}</span><b>{sector}</b></header><h3>{title}</h3><p>{body}</p><div className="solutions-scenario-detail"><div><small>{aLabel}</small><strong>{aText}</strong></div><div><small>{eLabel}</small><strong>{eText}</strong></div><div><small>{rLabel}</small><strong>{rText}</strong></div></div></article>; })}</div></div></section>
      ...
      <section className="solutions-section solutions-section--workflow"><div className="shell-content"><div className="solutions-heading"><div className="solutions-kicker"><span/>{t.workflowTag}</div><h2>{t.workflowTitleA}<br/><em>{t.workflowTitleB}</em></h2><p>{t.workflowBody}</p></div><div className="solutions-workflow">{t.workflow.map(([n,title,question,body])=><article key={n}><span>{n}</span><small>{title}</small><h3>{question}</h3><p>{body}</p></article>)}</div></div></section>
